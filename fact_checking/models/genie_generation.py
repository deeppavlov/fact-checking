...
from .constrained_generation import (
    get_information_extraction_prefix_allowed_tokens_fn_hf,
)
...
class GenieHF(BartForConditionalGeneration):
    @classmethod
    def from_pretrained(
        cls, model_name_or_path, return_dict=True, other_parameters=None
    ):
...
def convert_output_to_triplets(output_obj):
    ...
    for sample in output_obj:
        sample["textual_triplets"] = TripletUtils.convert_text_sequence_to_text_triples(
            sample["text"]
        )
    return output_obj
...
class GenIEBARTGeneration(TorchModel):
    ...
    def sample(self, input_data, prefix_allowed_tokens_fn=None, **kwargs):
        ...
        if self.generation_params.get("return_dict_in_generate", False):
            output["sequences"] = self.tokenizer.batch_decode(
            ...
            for seqs, scores in batch:
                output_obj = [
                    {"text": seq, "log_prob": score} for seq, score in zip(seqs, scores)
                ]
                if self.convert_to_triplets:
                    output_obj = convert_output_to_triplets(output_obj)
                output_obj = sorted(
                    output_obj, key=lambda x: x["log_prob"], reverse=True
                )
    ...
    def __call__(self, batch_x: Dict[str, torch.tensor]):
        ...
        if self.generation_params.get("return_dict_in_generate"):
            ...
            for preds in outputs:
                top_preds = []
                for idx, lpred in enumerate(preds):
                    score = lpred.get("log_prob", -100)
                    pred = lpred.get("textual_triplets", [])
                    if score > self.thresh and not (
                        pred[0] == "unk" and pred[2] == "unk"
                    ):
                        top_preds.append(pred)
                    if idx == 0 and not top_preds:
                        if len(pred) == 1 and pred[0][0] == "unk":
                            pred = [(p[2], p[1], p[0]) for p in pred]
                        top_preds.append(pred)
    ...
    def load(self, fname: str = None):
        ...
        log.info("Everything loaded succesfully!")
